        ...
        if (apiKey) {
            ...
            const response = await fetch(`${GEMINI_API_URL}?key=${apiKey}`, {
                ...
                body: JSON.stringify({
                  ...
                  Do not use Markdown. Return ONLY a JSON array of hex color codes without any explanation.`,
                                },
                            ],
                        },
                    ],
            ...
            try {
                ...
                if (
                    ...
                    colors.length !== 10 ||
                    !colors.every((c) => /^#[0-9A-F]{6}$/i.test(c))
                ) {
...
function generateMockColors(prompt: string): string[] {
    ...
    for (const theme in themeColors) {
        if (
            lowercasePrompt.includes(theme) ||
            (theme === 'ocean' &&
                (lowercasePrompt.includes('blue') ||
                    lowercasePrompt.includes('water') ||
                    lowercasePrompt.includes('sea'))) ||
            (theme === 'sunset' &&
                (lowercasePrompt.includes('orange') ||
                    lowercasePrompt.includes('warm') ||
                    lowercasePrompt.includes('evening'))) ||
            (theme === 'forest' &&
                (lowercasePrompt.includes('green') ||
                    lowercasePrompt.includes('nature') ||
                    lowercasePrompt.includes('trees'))) ||
            (theme === 'pastel' &&
                (lowercasePrompt.includes('soft') ||
                    lowercasePrompt.includes('light') ||
                    lowercasePrompt.includes('gentle'))) ||
            (theme === 'neon' &&
                (lowercasePrompt.includes('bright') ||
                    lowercasePrompt.includes('vibrant') ||
                    lowercasePrompt.includes('night'))) ||
            (theme === 'vintage' &&
                (lowercasePrompt.includes('retro') ||
                    lowercasePrompt.includes('old') ||
                    lowercasePrompt.includes('classic')))
        ) {
            selectedTheme = theme
            break
        }
